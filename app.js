...
// --- 기본 프로필 아바타 (이미지 로드 실패 시 폴백) ---
const DEFAULT_AVATAR_SVG = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23555'%3E%3Cpath d='M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z'/%3E%3C/svg%3E";

function setProfileImgSrc(imgEl, url) {
    if (!imgEl) return;
    imgEl.onerror = function() { this.onerror = null; this.src = DEFAULT_AVATAR_SVG; };
    imgEl.src = url;
}
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if(data.photoURL) {
                ...
                setProfileImgSrc(document.getElementById('profilePreview'), data.photoURL);
                ...
                if (isBase64Image(data.photoURL) && auth.currentUser) {
                    uploadImageToStorage(`profile_images/${auth.currentUser.uid}/profile.jpg`, data.photoURL)
                        .then(downloadURL => {
                            ...
                            setProfileImgSrc(document.getElementById('profilePreview'), downloadURL);
                            saveUserData();
                        })
            ...
            if (user.photoURL) {
                ...
                setProfileImgSrc(document.getElementById('profilePreview'), user.photoURL);
            }
            await saveUserData();
        }
        loadPlayerName();
    ...
    container.innerHTML = list.map((u, i) => `
        <div class="user-card ${u.isMe ? 'my-rank' : ''}">
            ...
            <div style="display:flex; align-items:center; flex-grow:1; margin-left:10px;">
                ${u.photoURL ? `<img src="${sanitizeURL(u.photoURL)}" onerror="this.onerror=null;this.src='${DEFAULT_AVATAR_SVG}';" style="width:30px; height:30px; border-radius:50%; object-fit:cover; margin-right:8px; border:1px solid var(--neon-blue);">` : `<div style="width:30px; height:30px; border-radius:50%; background:#444; margin-right:8px; border:1px solid var(--neon-blue);"></div>`}
...
async function loadProfileImage(event) {
    ...
    reader.onload = (e) => {
        ...
        img.onload = async () => {
            ...
            setProfileImgSrc(document.getElementById('profilePreview'), base64);
            try {
    ...
    const html = posts.map(post => {
        ...
        return `<div class="system-card reels-card" data-post-id="${postId}">
            <div class="reels-header">
                <img class="reels-avatar" src="${profileSrc}" onerror="this.onerror=null;this.src='${DEFAULT_AVATAR_SVG}';" alt="">
    ...
    if (comments.length === 0) {
        ...
        container.innerHTML = comments.map(c => {
            ...
            return `<div class="reels-comment-item">
                <img class="reels-comment-avatar" src="${cPhoto}" onerror="this.onerror=null;this.src='${DEFAULT_AVATAR_SVG}';" alt="">
