...
(function() {
    ...
    // ── TMDB API (Cloud Function 경유 — library.js 패턴) ──
    ...
    async function fetchMovieDetails(tmdbId) {
        log('상세 조회: tmdbId=' + tmdbId);
        try {
            var _ping = window._httpsCallable(window._functions, 'ping');
            var result = await _ping({ action: 'lookupMovie', tmdbId: String(tmdbId) });
            var data = result.data || {};
            var movie = data.movie || null;
            if (movie) {
                log('상세 조회 성공: ' + movie.title);
            } else {
                logWarn('상세 조회 결과 없음: tmdbId=' + tmdbId);
            }
            return movie;
        } catch(e) {
            var errMsg = (e && e.message) || String(e);
    ...
    window.showMovieAddOptions = async function(tmdbId) {
        ...
        if (!detail) {
            logWarn('상세정보 로드 실패로 추가 불가: tmdbId=' + tmdbId);
