export function createQuestStatsModule(deps) {
    ...
    function renderQstatsCalendar() {
        ...
        container.innerHTML = Array.from({ length: 7 }, (_, i) => {
            ...
            const diyDefs = AppState.diyQuests?.definitions || []; const diyDoneMap = AppState.diyQuests?.completedToday || {}; const diyCount = diyDefs.length; let count = 0, total = 12;
            if (isToday) { const dow = AppState.quest?.currentDayOfWeek ?? new Date().getDay(); const s = AppState.quest?.completedState?.[dow] || []; count = s.filter(v => v).length + Object.values(diyDoneMap).filter(v => v).length; total = 12 + diyCount; }
    ...
    function getDailyQuestDoneTotal(dow, idx, rec, isToday) {
        if (isToday) {
            const dow = AppState.quest?.currentDayOfWeek ?? new Date().getDay(); const stateArr = AppState.quest?.completedState?.[dow] || [];
    ...
    function renderMonthlyHeatmap(year, month, history) {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            else if (isToday) { const diyCount = (AppState.diyQuests?.definitions || []).length; const dow = AppState.quest?.currentDayOfWeek ?? new Date().getDay(); const regularDone = (AppState.quest?.completedState?.[dow] || []).filter(v => v).length; const diyDone = Object.values(AppState.diyQuests?.completedToday || {}).filter(v => v).length; done = state.diyOnly ? diyDone : (regularDone + diyDone); total = state.diyOnly ? diyCount : (12 + diyCount); const rate = total > 0 ? done / total * 100 : 0; level = rate >= 76 ? 4 : rate >= 51 ? 3 : rate >= 26 ? 2 : rate >= 1 ? 1 : 0; }
            ...
            series.push({
                ...
                values: labels.map((k) => {
                    ...
                    if (k === todayStr) {
                        ...
                        const dow = AppState.quest?.currentDayOfWeek ?? new Date().getDay(); const regularDone = (AppState.quest?.completedState?.[dow] || []).filter(v => v).length;
